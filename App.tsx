...
const App: React.FC = () => {
  ...
  const checkWriting = async () => {
    if (userSelections.story.length < 20) {
      setLanguageFeedback({ type: 'tip', text: "Your log is a bit short! Try to add a few more details about what happened." });
      return;
    }
    ...
    try {
      ...
      const selectedWords = Object.values(userSelections.selectedVocabulary).flat().join(', ');
      
      const prompt = `
        Act as a friendly, encouraging professional writing mentor for an ESL student at the Higher Colleges of Technology (UAE). 
        The student is writing an "Interview Hero Log" about their master skill: "${topCategoryData?.name}".
        They are using the STAR (Situation, Task, Action, Result) method.
        
        Student's Story: "${userSelections.story}"
        Target Vocabulary they wanted to use: [${selectedWords}]

        Provide 2-3 short, constructive "Heroic Tips" to improve their story. 
        Focus on:
        1. Whether they used Past Tense correctly (essential for interview stories).
        2. Whether they clearly explained their ACTION (what THEY did) and the RESULT (the positive outcome).
        3. Encouraging them to use their target vocabulary words.
        
        Keep the tone gamified and positive. Use bullet points. Keep it under 80 words.
      `;
      ...
      const feedback = response.text || "Scanning successful! Your story looks like a great start. Remember to focus on your specific actions!";
      setLanguageFeedback({ 
        type: feedback.toLowerCase().includes('great') || feedback.toLowerCase().includes('excellent') ? 'good' : 'tip', 
        text: feedback 
      });
    } catch (error) {
      console.error("AI Error:", error);
      setLanguageFeedback({ type: 'tip', text: "Bio-scanner interference! Just remember to use past tense (I helped, I did) and explain your results clearly." });
    } finally {
      setIsScanning(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-white pb-12">
        ...
        <header className="sticky top-0 z-50 bg-slate-900/80 backdrop-blur-md border-b border-white/10 px-6 py-4 no-print shadow-2xl">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white">Higher Colleges of Technology</span>
            </div>
            ...
            <div className="flex-1 max-w-sm mx-8 hidden sm:block">
              ...
              <div className="h-3 w-full bg-slate-800 rounded-full overflow-hidden border border-white/5 p-0.5">
                <div 
                  className="h-full bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500 rounded-full transition-all duration-700 ease-out"
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 pt-8">
              ...
              {currentCategory.items.map((item, idx) => (
                <div key={item.id} className="glass-card rounded-[2rem] overflow-hidden border border-white/10 p-8 hover:bg-white/[0.07] transition-all group">
                   <div className="flex gap-4 mb-6">
                      <span className="text-blue-300 font-black text-2xl bungee opacity-100 drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]">0{idx + 1}</span>
                      ...
                      <button
                        ...
                        className={`group relative flex flex-col items-center gap-2 py-5 rounded-2xl transition-all border-2 ${
                          userSelections.ratings[item.id] === score
                            ? 'bg-blue-600 border-blue-300 shadow-[0_0_20px_rgba(59,130,246,0.6)] scale-[1.02]'
                            : 'bg-slate-800/60 border-white/10 hover:border-white/30'
                        }`}
                      >
                        <span className={`text-2xl font-black ${userSelections.ratings[item.id] === score ? 'text-white' : 'text-slate-100'}`}>
                          {score}
                        </span>
                        <span className={`text-[9px] font-black uppercase tracking-tighter ${userSelections.ratings[item.id] === score ? 'text-white' : 'text-white/80'}`}>
        ...
        {currentStep === 'vocabulary' && (
          <div className="animate-in fade-in slide-in-from-bottom-12 duration-700">
            <div className="text-center mb-12">
              ...
              <h2 className="text-5xl bungee tracking-tight mb-4 text-white">SKILL INVENTORY</h2>
              <p className="text-slate-300 max-w-xl mx-auto font-medium">Equip your professional keywords! Click to add them to your hero profile.</p>
            </div>
                      ...
                      return (
                        <button
                          ...
                          className={`px-5 py-3 rounded-2xl font-black text-sm border-2 transition-all flex items-center gap-2 ${
                            isSelected
                              ? `${cat.color} text-white border-transparent shadow-[0_0_20px_rgba(255,255,255,0.2)] scale-105`
                              : 'bg-slate-800/40 text-slate-200 border-white/10 hover:border-white/30 hover:text-white'
                          }`}
                        >
            ...
            <div className="mt-12 glass-card rounded-[2.5rem] p-10 border-2 border-indigo-500/30 text-center bg-gradient-to-br from-indigo-900/20 to-purple-900/20">
              ...
              <p className="text-indigo-100 font-bold uppercase text-[10px] tracking-[0.3em] mb-10">You can only master one main skill for your story</p>
                   ...
                   <button
                    ...
                    className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 relative overflow-hidden group ${
                      userSelections.topStrengthId === cat.id
                        ? 'bg-white text-indigo-900 border-white scale-105 shadow-[0_0_40px_rgba(255,255,255,0.4)]'
        ...
        {currentStep === 'story' && (
          <div className="animate-in zoom-in-95 duration-500">
            <div className="text-center mb-10">
              <h2 className="text-5xl bungee tracking-tight mb-4 text-white">HERO LOG</h2>
              <p className="text-slate-300 font-medium">"Tell me about a time you used your <span className="text-blue-400 font-black">{topCategoryData?.name}</span> power."</p>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 mb-8">
                ...
                <div className="glass-card rounded-[2.5rem] border border-white/10 overflow-hidden">
                  <div className="bg-slate-800/80 px-8 py-5 flex justify-between items-center border-b border-white/5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="ml-4 text-[10px] font-black uppercase tracking-widest text-slate-300">Terminal V1.0</span>
                    </div>
                    <button 
                      onClick={() => setShowToolbox(!showToolbox)}
                      className="text-[10px] font-black uppercase text-blue-400 hover:text-white transition-all bg-blue-500/20 px-4 py-1.5 rounded-full ring-1 ring-blue-500/40"
                    >
                  ...
                  <div className="p-8">
                    <textarea 
                      ...
                      className="w-full h-80 p-6 rounded-2xl bg-black/40 border-2 border-white/10 focus:border-blue-500/50 focus:ring-0 transition-all text-xl resize-none font-medium placeholder:text-slate-600 text-white"
                    />
                    
                    <div className="mt-6 flex flex-wrap gap-4 items-center justify-between">
                       <button 
                        onClick={checkWriting}
                        disabled={isScanning || userSelections.story.length < 10}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-2xl text-sm font-black flex items-center gap-3 transition-all active:scale-95 disabled:opacity-30 shadow-lg"
                      >
                        {isScanning ? (
                          <>
                            <Loader2 className="w-5 h-5 animate-spin" /> SCANNING LOG...
                          </>
                        ) : (
                          <>
                      ...
                      <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest bg-slate-800/50 px-4 py-2 rounded-lg border border-white/10">
                        Status: <span className={userSelections.story.length >= 30 ? 'text-green-400' : 'text-yellow-400'}>
                    ...
                    {languageFeedback && (
                      <div className={`mt-8 p-6 rounded-[2rem] flex items-start gap-4 animate-in slide-in-from-top-4 ${
                        languageFeedback.type === 'good' ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-50' : 'bg-orange-500/10 border border-orange-500/30 text-orange-50'
                      }`}>
                        ...
                        <div className="flex-1">
                          <p className="text-xs font-black uppercase tracking-widest mb-1 opacity-80">
                            {languageFeedback.type === 'good' ? "MISSION INTEL" : "HEROIC TIPS"}
                          </p>
                          <div className="font-bold text-lg leading-tight whitespace-pre-wrap">
              ...
              <div className={`lg:w-80 space-y-4 transition-all duration-300 ${showToolbox ? 'opacity-100 translate-x-0' : 'lg:opacity-100'}`}>
                <div className="glass-card rounded-[2.5rem] p-8 border border-white/10 h-full">
                  <h4 className="font-black text-blue-300 text-[11px] uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                    <Languages className="w-4 h-4" /> PHRASE BOOSTER
                  </h4>
                  <div className="space-y-6">
                    {sentenceStarters.map((starter) => (
                      <div key={starter.label} className="space-y-2">
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest px-2">{starter.label}</span>
                        <div className="flex flex-col gap-2">
                          {starter.phrases.map((phrase) => (
                            <button
                              ...
                              className="text-left text-[11px] font-bold p-3 bg-white/10 hover:bg-blue-500/30 border border-white/10 rounded-xl text-slate-100 hover:text-white transition-all hover:scale-[1.02]"
                            >
                              {phrase}
                            </button>
                          ))}
            ...
            <div className="flex justify-between items-center pb-12">
              <button 
                ...
                className="px-8 py-5 rounded-2xl text-slate-300 font-black hover:bg-slate-800/50 transition-all border border-white/10"
              >
                BACK
              </button>
              <button 
                ...
                className={`px-16 py-6 rounded-[2rem] font-black text-2xl bungee tracking-widest shadow-[0_0_40px_rgba(79,70,229,0.4)] transition-all ${
        ...
        {currentStep === 'summary' && (
          <div className="animate-in zoom-in-90 duration-1000 pb-20">
            <div className="max-w-3xl mx-auto glass-card rounded-[3rem] overflow-hidden border-4 border-white/20 shadow-[0_0_100px_rgba(59,130,246,0.3)] print:border-slate-200 print:shadow-none print:rounded-none">
                ...
                <div className="relative z-10">
                  <div className="flex flex-col items-center gap-1 mb-8">
                     <div className="px-4 py-1.5 bg-white/10 backdrop-blur-md rounded-full border border-white/20">
                        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-100">LSC 1503 Interview Profile</span>
                     </div>
                     <span className="text-[11px] font-black uppercase tracking-[0.2em] text-white mt-1 drop-shadow-sm">Higher Colleges of Technology</span>
                  </div>

                  <div className="mb-8 floating">
                    <div className="relative inline-block">
                      <div className="absolute inset-0 bg-white/30 blur-3xl rounded-full"></div>
                      <div className="relative w-32 h-32 bg-white rounded-[2.5rem] shadow-2xl flex items-center justify-center text-7xl transform -rotate-3 border-4 border-white/30">
                        {topCategoryData?.icon}
                      </div>
                    </div>
                  </div>

                  <h1 className="text-6xl md:text-7xl bungee tracking-tighter mb-4 text-white leading-none drop-shadow-2xl">QUEST COMPLETE!</h1>
                  <p className="text-blue-50 text-xl font-bold opacity-90 max-w-md mx-auto italic drop-shadow-md">
                    "I have the skills, the words, and the story to succeed."
                  </p>
                </div>
              </div>

              <div className="p-10 md:p-16 space-y-16 bg-slate-900/60 print:bg-white print:text-slate-900">
                <section>
                   <div className="flex items-center gap-3 mb-8">
                      <div className="w-12 h-1 bg-blue-400 rounded-full"></div>
                      <h3 className="text-xl font-black uppercase tracking-[0.3em] text-blue-300">Combat Class</h3>
                   </div>
                   <div className="bg-white/5 p-10 rounded-[2.5rem] border-2 border-white/10 print:bg-slate-50 print:border-slate-200">
                      <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                         <div className="text-center md:text-left">
                            <span className="text-blue-400 font-black uppercase text-xs tracking-widest">Master Category</span>
                            ...
                            <div className="flex items-center justify-center md:justify-start gap-2 mt-4">
                               {[1,2,3,4,5].map(s => (
                                  <Star key={s} className="w-5 h-5 text-yellow-300 fill-yellow-300" />
                               ))}
                               <span className="ml-2 text-xs font-black text-slate-300">MAX LEVEL</span>
                            </div>
                         </div>
                         <div className="bg-white/5 p-6 rounded-3xl border border-white/5 text-center px-10 print:bg-white print:border-slate-300">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Role Description</span>
                            <p className="text-lg font-bold text-white italic print:text-slate-700">"A reliable professional who drives success."</p>
                         </div>
                      </div>
                   </div>
                </section>
                ...
                <section>
                   <div className="flex items-center gap-3 mb-8">
                      ...
                      <h3 className="text-xl font-black uppercase tracking-[0.3em] text-purple-300">Battle Report</h3>
                   </div>
                   <div className="relative group">
                      <div className="absolute -inset-1 bg-gradient-to-r from-purple-500 to-blue-500 rounded-[2.5rem] blur opacity-30 group-hover:opacity-50 transition duration-1000"></div>
                      <div className="relative bg-black/50 backdrop-blur-xl p-12 rounded-[2.5rem] border-2 border-purple-400/40 print:bg-white print:border-slate-300 print:shadow-none">
                         <div className="flex items-center gap-2 text-[10px] font-black text-purple-300 uppercase tracking-[0.3em] mb-6">
                ...
                <section>
                   <div className="flex items-center gap-3 mb-8">
                      ...
                      <h3 className="text-xl font-black uppercase tracking-[0.3em] text-emerald-300">Power Keywords</h3>
                   </div>
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {CATEGORIES.filter(cat => cat.items.some(i => (userSelections.ratings[i.id] || 0) >= 4)).map(cat => (
                      <div key={cat.id} className="p-8 rounded-[2rem] bg-white/[0.05] border border-white/10 hover:bg-white/[0.08] transition-all print:bg-slate-50 print:border-slate-200">
                        <div className="flex items-center gap-3 mb-6">
                          <span className="text-2xl">{cat.icon}</span>
                          <span className="font-black text-white text-[10px] uppercase tracking-[0.3em]">{cat.name}</span>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          {(userSelections.selectedVocabulary[cat.id] || []).map(v => (
                            <span key={v} className="bg-emerald-500/30 px-4 py-2 rounded-xl text-xs font-black text-white border border-emerald-400/40 print:bg-white print:text-emerald-700 print:border-emerald-200 shadow-xl">
                              {v}
                            </span>
                          ))}
                        </div>
                ...
                <div className="pt-16 flex flex-col items-center gap-6 no-print border-t border-white/10">
                  <button 
                  ...
                  <button 
                    ...
                    className="text-slate-300 hover:text-white font-black text-xs uppercase tracking-[0.4em] transition-all"
                  >
              ...
              <div className="bg-slate-950 p-8 text-center border-t border-white/10 print:bg-white print:text-slate-400 print:border-slate-200">
                 <p className="text-[9px] font-black text-white/50 uppercase tracking-[0.5em] print:text-slate-400">
      ...
      <footer className="py-12 text-center text-slate-500 text-[9px] font-black uppercase tracking-[0.5em] no-print">
